...
* @param {String|Boolean} [opts.open] - command to open a destination directory or `false`
...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'open' ) ) {
		...
		if ( !isString( opts.open ) && opts.open !== false ) {
			return new TypeError( 'invalid option. Open option must be either a string primitive or `false`. Option: `' + opts.open + '`.' );
		}
	}
