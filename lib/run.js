...
var getPhases = require( './phases.js' );
...
function starter() {
	...
	*/
	function next( error ) {
		...
		log( 'All tasks for phase %d are complete.', idx+1 );
		idx += 1;
		if ( idx < nPhases ) {
			log( 'Beginning tasks for phase %d of %d...', idx+1, nPhases );
			return execTasks( phases[ idx ], next );
		}
