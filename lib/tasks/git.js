...
function git( debug ) {
	...
	*/
	return function init( dest, options, clbk ) {
		...
		log( 'Local git repository initialized: %s', out.stdout );
		...
		if ( options.repo ) {
			...
			log( 'Remote origin added: %s', out.stdout );
		}
		// Add local files:
		...
		log( 'Local files added: %s', out.stdout );
		...
		log( 'Initial commit made: %s', out.stdout );
